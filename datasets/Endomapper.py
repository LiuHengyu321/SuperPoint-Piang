...
class Endomapper(data.Dataset):
    ...
    def __init__(self, export=False, transform=None, task='train', **config):
        ...
        if self.config['labels']:
            ...
            print("load labels from: ", self.config['source1'], "and ",self.config['source2'])
            count = 0
            for (img, name) in zip(files['image_paths'], files['names']):
                p1 = Path(self.config['source1'], '{}.npz'.format(name))
                p2 = Path(self.config['source2'], '{}.npz'.format(name[:-4]))
                if p1.exists() and p2.exists():
                    sample = {'image': img, 'name': name, 'points': [str(p1), str(p2)]}
