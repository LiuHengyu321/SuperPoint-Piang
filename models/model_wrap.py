...
from utils.utils import to_numpy, flatten_detection
from utils.loader import model_loader
from utils.losses import soft_argmax_2d, norm_patches, do_log, extract_patch_from_points
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def loadModel(self, weights_path):
        ...
        if trained:
            ...
            print("model: ", model)
            self.net = model_loader(model=model, **params)
    ...
    def run(self, inp, onlyHeatmap=False, train=True):
        ...
        # flatten detection
        heatmap = flatten_detection(semi, tensor=True)
        ...
        if self.subpixel:
            labels_res = outs[2]
            self.pts_subpixel = [self.subpixel_predict(to_numpy(labels_res[i, ...]), pts[i]) for i in range(batch_size)]
        '''
        pts:
