...
from utils.loader import data_loader, model_loader, pretrainedLoader
import logging
from pathlib import Path

from utils.tools import dict_update

from utils.utils import labels_2d_to_3d, flatten_detection, labels2Dto3D_flattened
from utils.utils import pltImshow, save_image, save_checkpoint, box_nms
from utils.utils import precision_recall_torch, get_pts_from_heatmap
...
class Train_model_frontend(object):
    ...
    def loadModel(self):
        ...
        print("model: ", model)
        net = model_loader(model=model, **params).to(self.device)
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        if self.subpixel:
            ...
            dense_map = flatten_detection(semi_warp)  # tensor [batch, 1, H, W]
    ...
    def addImg2tensorboard(
        ...
        n_iter = self.n_iter
        semi_flat = flatten_detection(semi[0, :, :, :])
        semi_warp_flat = flatten_detection(semi_warp[0, :, :, :])
    ...
    def add2tensorboard_nms(self, img, labels_2D, semi, task="training", batch_size=1):
        ...
        for idx in range(batch_size):
            semi_flat_tensor = flatten_detection(semi[idx, :, :, :]).detach()
            semi_flat = toNumpy(semi_flat_tensor)
            semi_thd = np.squeeze(semi_flat, 0)
            pts_nms = get_pts_from_heatmap(semi_thd, conf_thresh, nms_dist)
            ...
            precision_recall = precision_recall_torch(
            ...
            if boxNms:
                ...
                precision_recall_boxnms = precision_recall_torch(
    ...
    def get_heatmap(self, semi, det_loss_type="softmax"):
        if det_loss_type == "l2":
            ...
            heatmap = flatten_detection(semi)
        return heatmap
